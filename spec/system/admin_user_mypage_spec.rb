...
describe '講師(マイページ)のテスト' do
  describe '講師マイページ(投稿作品以外)のテスト' do
    ...
    before do
      create_list(:picture, 4, :comic, user: user_paid, genre: genre)
    ...
    context '表示の確認' do
      ...
      it '個人情報編集リンクが表示される' do
        expect(page).to have_link '登録情報を編集する', href: edit_user_path(user_admin)
      end
      it '作品投稿リンクが表示される' do
    ...
    context '他人(有料会員)のマイページの確認' do
      ...
      it '個人情報が表示される' do
        ...
        expect(page).to have_content('氏名(かな)')
    ...
    context '他人(退会有料会員)のマイページ遷移の確認' do
      it '画面遷移できる' do
        visit user_path(deleted_user_paid)
        expect(current_path).to eq(user_path(deleted_user_paid))
      end
    end
    ...
    context '他人(無料会員)のニックネーム表示確認' do
      ...
      context '他人(退会無料会員)のマイページ遷移の確認' do
        it '画面遷移できる' do
          visit user_path(deleted_user_free)
          expect(current_path).to eq(user_path(deleted_user_free))
        end
      end
    end
    ...
    context '他人(講師)のマイページの確認' do
      ...
      it 'ステータスが表示される' do
        expect(page).to have_content(user_admin2.status)
      end
      it 'ニックネームが登録されていないとき氏名が表示される' do
        expect(page).to have_content(user_admin2.name)
      end
      it '編集リンクが表示されない' do
        expect(page).to have_no_link '登録情報を編集する', href: edit_user_path(user_admin2)
      end
      it 'お気に入りリンクが表示されない' do
        expect(page).to have_no_link 'お気に入り', href: bookmarks_path
      end
    ...
    context '他人(講師)のニックネーム表示確認' do
      ...
      it 'ニックネームが表示される' do
        expect(page).to have_content(user_admin2.nickname)
      end
    end

    context '他人(退会講師)のマイページ遷移の確認' do
      it '画面遷移できる' do
        visit user_path(deleted_user_admin)
        expect(current_path).to eq(user_path(deleted_user_admin))
      end
    end
  end
  ...
  describe '有料会員マイページ(投稿作品)のテスト' do
    ...
    before do
      ...
      create(:picture, :comic, user: user_admin3, genre: genre)
      create(:picture, :comic, user: user_admin4, genre: genre)
      create(:picture, :illustration, user: user_paid2, genre: genre)
      create(:picture, :illustration, user: user_admin3, genre: genre)
      create(:picture, :illustration, user: user_admin4, genre: genre)
      visit new_user_session_path
  ...
  describe '有料会員マイページ(マンガ「もっと見る」表示確認)のテスト' do
    ...
    let!(:user_admin5) { create(:user, :admin) }
    let!(:user_admin6) { create(:user, :admin) }
    let!(:genre) { create(:genre, :active) }

    before do
      create_list(:picture, 4, :comic, user: user_paid3, genre: genre)
      create_list(:picture, 4, :comic, user: user_admin5, genre: genre)
      create_list(:picture, 4, :comic, user: user_admin6, genre: genre)
      visit new_user_session_path
      fill_in 'user[email]', with: user_admin5.email
      fill_in 'user[password]', with: user_admin5.password
      click_button 'ログインする'
    end
    ...
    context '他人(有料会員)の表示確認' do
      ...
      it '画面遷移できる' do
        ...
        expect(page).to have_no_content(user_paid3.name)
        expect(page).to have_no_content(user_admin6.name)
      end
    end

    context '他人(講師)の表示確認' do
      before do 
        visit user_path(user_admin6)
      end
      it '「もっと見る」が表示される' do
        expect(page).to have_content('もっと見る')
      end
  ...
  describe '有料会員マイページ(イラスト「もっと見る」表示確認)のテスト' do
    ...
    let!(:user_admin7) { create(:user, :admin) }
    let!(:user_admin8) { create(:user, :admin) }
    let!(:genre) { create(:genre, :active) }

    before do
      create_list(:picture, 4, :illustration, user: user_paid4, genre: genre)
      create_list(:picture, 4, :illustration, user: user_admin7, genre: genre)
      create_list(:picture, 4, :illustration, user: user_admin8, genre: genre)
      visit new_user_session_path
      fill_in 'user[email]', with: user_admin7.email
      fill_in 'user[password]', with: user_admin7.password
      click_button 'ログインする'
    end
  ...
  describe '有料会員マイページ(ジャンル無効作品)のテスト' do
    let!(:user_admin9) { create(:user, :admin) }
    let!(:deleted_genre) { create(:genre, :delete) }
    let!(:picture1) { create(:picture, :illustration, user: user_admin9, genre: deleted_genre) }
    let!(:picture2) { create(:picture, :illustration, user: user_admin9, genre: deleted_genre) }

    before do
      visit new_user_session_path
      fill_in 'user[email]', with: user_admin9.email
      fill_in 'user[password]', with: user_admin9.password
      click_button 'ログインする'
    end
